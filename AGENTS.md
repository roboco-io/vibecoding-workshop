...
- **Part 2**: 모던 소프트웨어 엔지니어링 — David Farley 원칙, Codex 심화, OMC 소개
...
- **대상**: 개발자 15명 (전원 시니어, Codex 100% 사용 중)
- **시간**: 4시간 (14:00~18:00)
...
- **도구**: Codex (YOLO 모드) + OMC + OpenSpec + GitHub CLI
...
| 컨텍스트 부족이 장애물 | 60% | 인터뷰 퍼스트 패턴 + AGENTS.md |
| Codex 100% 사용 중 | 100% | 기초 설명 생략, 실전 중심 |
...
이 프로젝트는 OpenSpec을 사용하여 변경 관리를 수행한다. `openspec/` 디렉토리에 프로젝트 컨텍스트와 기능별 스펙이 관리되며, `.github/skills/` 및 `common/.Codex/skills/`에 OpenSpec 관련 스킬이 정의되어 있다.
...
- `openspec/project.md`는 AGENTS.md와 동기화 유지 — 프로젝트 구조 변경 시 양쪽 모두 업데이트
