...
**핵심 메시지**: "검증된 비개발자용 바이브 코딩 프로세스" — 일회성 이벤트가 아닌 반복 운영되는 정규 프로세스

**레이아웃**: 3단 그리드

| 워크숍 운영 | 검증 이력 | 사전 준비 |
|------------|----------|----------|
| 인원: 최대 20명 | 운영 이력: 반복 운영 | Claude 계정 |
| 시간: 4시간 | 보안 검토: 완료 | 노트북 |
| 형식: 온라인/오프라인 | 인증·확장성: 검토 완료 | 사내 데이터 정책 확인 |
| 사전 준비: 1주 전 | 실제 업무 사용: 적용 중 | 샘플 자료 |
...
| 타이포 | 시스템 폰트, 본문 19px, 헤더 22~27px (1600px 캔버스 기준 — 축소 표시 시 가독성 확보) |
